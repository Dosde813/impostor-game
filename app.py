...
app = Flask(__name__)
app.secret_key = "impostor_elite_v2_1"
...
DICCIONARIO_TOTAL = {
    ...
    "Zapato": "Pie", "Sombrero": "Cabeza", "Gafas": "Ojos", "Guantes": "Manos"
}
...
<head>
    ...
    <title>Impostor Elite V2.1</title>
...
<body>
    ...
    <script>
        ...
        let miToken = localStorage.getItem('elite_tk_v21');
        let miNombre = localStorage.getItem('elite_nm_v21');
        ...
        function registrar() {
            ...
            if(n) {
                ...
                localStorage.setItem('elite_tk_v21', miToken);
                localStorage.setItem('elite_nm_v21', miNombre);
        ...
        socket.on('pantalla_lobby', (data) => {
            ...
            if(data.ultimo_res) {
                ...
                document.getElementById('res-txt').innerHTML = "Impostor era: <b style='color:#ef4444'>" + data.ultimo_res.nombre + "</b><br>Palabra era: <b>" + data.ultimo_res.palabra + "</b>";
            } else {
                document.getElementById('resultado-previo').classList.add('hidden');
            }
        });
...
def cerrar():
    game.update({"encendido": False, "estado": "lobby", "jugadores": {}, "roles": {}, "ultimo_resultado": None})
