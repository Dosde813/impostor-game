...
app = Flask(__name__)
app.secret_key = "impostor_v14_silent"
...
<body>
    <div class="box">
        ...
        <div id="sec-admin" class="hidden">
            <button class="btn" style="background:#00ff88; color:#000;" onclick="socket.emit('activar')">ENCENDER</button>
        ...
        <div id="sec-lobby" class="hidden">
            <div id="lista"></div>
            <button id="btn-iniciar" class="btn hidden" style="background:#00ff88; color:#000;" onclick="socket.emit('iniciar')">INICIAR</button>
        </div>
        ...
        <div id="sec-juego" class="hidden">
            ...
            <button id="btn-fin" class="btn hidden" style="background:#ffcc00; color:#000;" onclick="socket.emit('finalizar')">REVELAR</button>
        </div>
    </div>
    ...
    <script>
        ...
        let miToken = localStorage.getItem('imp_token_v14');
        let miNombre = localStorage.getItem('imp_name_v14');
        ...
        function registrar() {
            ...
            if(n) {
                ...
                localStorage.setItem('imp_token_v14', miToken);
                localStorage.setItem('imp_name_v14', miNombre);
        ...
        socket.on('estado_servidor', (data) => {
            if(!data.encendido) {
                ...
                localStorage.removeItem('imp_token_v14');
            }
        });
...
def handle_reconectar(data):
    ...
    # Si el jugador es nuevo o cambió de conexión, actualizamos su SID
    if token:
        game['jugadores'][token] = {'nombre': nombre, 'sid': request.sid}
        
        # SI LA PARTIDA ESTÁ EN CURSO: Le mandamos su rol SOLO A ÉL
        if game['estado'] == "juego" and nombre in game['roles']:
            emit('ver_rol', game['roles'][nombre])
        # SI ESTÁN EN EL LOBBY: Actualizamos la lista para todos
        elif game['estado'] == "lobby":
            nombres = [j['nombre'] for j in game['jugadores'].values()]
            socketio.emit('pantalla_lobby', {'nombres': nombres})
    else:
        emit('pantalla_registro')
...
def iniciar():
    ...
    for tk in tokens:
        ...
        game['roles'][nombre] = info # Guardamos por NOMBRE para persistencia
